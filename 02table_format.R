...
command10 <- paste0("awk 'BEGIN {FS=\"\\t\"} NR==FNR {a[$1]; next} FNR==1 || ($1 in a) {print}' /tmp/chrpos_allele_table.tsv /tmp/", args$pop, "_full_", args$phecode, ".tsv > /tmp/gtex_", args$phecode, ".tsv")
system(command10)
